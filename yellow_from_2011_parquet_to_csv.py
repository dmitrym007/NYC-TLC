...
st = time.time()
columns_yellow = ['VendorID', 'tpep_pickup_datetime', 'tpep_dropoff_datetime', 'PULocationID', 'DOLocationID', 'RatecodeID']
columns_yellow_new = ['vendor_id', 'pickup_datetime', 'dropoff_datetime', 'pickup_location_id', 'dropoff_location_id', 'rate_code_id']

for file in os.listdir('data'):
    if file.split('.')[-1] == 'parquet' and file[:6] == 'yellow' and int(file.split('-')[0].split('_')[-1]) > 2010 and f"{file.split('.')[0]}.csv" not in os.listdir('/var/lib/clickhouse/user_files'):
        file_name = file.split('.')[0]
        file_path = f"{curr_dir}/data/{file_name}.parquet"      
        print(f"Start for {file_name}...")
        df_yellow = pl.read_parquet(file_path)
        df_yellow = df_yellow.lazy()\
        .with_columns([(pl.lit('yellow')).alias('car_type'),
                       ...
                       (pl.lit(None).alias('ehail_fee')),
                       (pl.lit(None).alias('trip_type')),
        ...
        .rename(dict(zip(columns_yellow, columns_yellow_new)))\
        .collect()
#         Write csv files to clickhouse user_files directory        
        df_yellow.write_csv(f"/var/lib/clickhouse/user_files/{file_name}.csv")
